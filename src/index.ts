...
import {
    captureScreenshot,
    closeBrowser,
} from './internal/screenshotCapture.js';
    ...
    .action(async (url: string, options) => {
        try {
            ...
            if ('tiles' in result) {
                ...
                console.error(
                    `📐 Full page dimensions: ${tiledResult.fullWidth}x${tiledResult.fullHeight}`
                );
                console.error(
                    `🔲 Created ${tiledResult.tiles.length} tiles of ${tiledResult.tileSize}x${tiledResult.tileSize} each`
                );
                ...
                if (options.output) {
                    ...
                    for (const tile of tiledResult.tiles) {
                        ...
                        console.error(
                            `Saved tile ${tile.row},${tile.col} to: ${filename} (${tile.width}x${tile.height})`
                        );
                    }
                } else {
                    console.error(
                        'Error: Full page tiled output requires -o/--output flag'
                    );
                    process.exit(1);
                }
            } else {
