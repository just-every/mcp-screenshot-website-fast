    ...
    it('should start MCP server without errors', async () => {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            serverProcess.stderr.on('data', data => {
                ...
                if (
                    stderr.includes('[INFO] [MCP] MCP Server starting up') ||
                    stderr.includes(
                        '[INFO] [MCP] MCP server connected and running successfully!'
                    ) ||
                    stderr.includes('[INFO] [MCP] Ready to receive requests') ||
                    stderr.includes(
                        '[DEBUG] [MCP] Creating MCP server instance'
                    )
                ) {
                    if (!resolved) {
                        resolved = true;
                        ...
                        setTimeout(() => {
                            ...
                            expect(stderr).toMatch(
                                /\[INFO\] \[MCP\] MCP Server starting up|\[INFO\] \[MCP\] MCP server connected and running successfully!|\[INFO\] \[MCP\] Ready to receive requests/
                            );
                            resolve();
                        }, 100);
                    }
            ...
            setTimeout(() => {
                if (!resolved) {
                    ...
                    reject(
                        new Error(
                            `Server did not start within ${timeout}ms. stderr: ${stderr}, stdout: ${stdout}`
                        )
                    );
                }
            }, timeout);
            ...
            serverProcess.on('exit', (code, signal) => {
                if (!resolved && code !== 0) {
                    ...
                    reject(
                        new Error(
                            `Server exited with code ${code}, signal ${signal}. stderr: ${stderr}, stdout: ${stdout}`
                        )
                    );
                }
            });
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            setTimeout(() => {
                if (!resolved) {
                    ...
                    console.error(
                        '[Test Error - bin] Captured stderr:',
                        stderr
                    );
                    console.error(
                        '[Test Error - bin] Captured stdout:',
                        stdout
                    );
                    reject(
                        new Error(
                            `Server did not start via bin within ${timeout}ms. stderr: ${stderr}, stdout: ${stdout}`
                        )
                    );
                }
            }, timeout);
            ...
            binProcess.on('exit', (code, signal) => {
                if (!resolved && code !== 0) {
                    ...
                    reject(
                        new Error(
                            `Bin process exited with code ${code}, signal ${signal}. stderr: ${stderr}, stdout: ${stdout}`
                        )
                    );
                }
            });
